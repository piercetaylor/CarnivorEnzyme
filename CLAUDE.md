...
| Phylogeny | IQ-TREE 3 | 3.0+ | ModelFinder Plus (`-m MFP`) for automatic model selection. UFBoot2 for ultrafast bootstrap. IQ-TREE 3 (2025) adds improved site concordance factors that reduce homoplasy effects in ancestral reconstruction — directly relevant to Phase 3. `piqtree` Python bindings enable clean integration with detect_convergence.py. | Minh et al. 2025, EcoEvoRxiv (IQ-TREE 3); Minh et al. 2020, *Mol. Biol. Evol.* 37:1530 | UFBoot2 values are NOT directly comparable to standard bootstrap; treat ≥95 as strong support. Use `-alrt 1000` alongside UFBoot2 for more reliable support. |
...
| Structure prediction (primary) | **AlphaFold3** | 3.0.1 | Confirmed available on Hellbender (A100 + H100 nodes). Gold standard; highest average accuracy across protein types (FoldBench 2025, 1522 assemblies). CC-BY-NC-SA 4.0 — qualifies for research use. Two-stage submission: CPU MSA (~2 h) then GPU inference (~2 h/seed). | Abramson et al. 2024, *Nature* 630:493; Xu et al. 2025, *Nature Communications* (FoldBench) | Weights restricted (CC-BY-NC-SA 4.0). A100 or newer required. Two SLURM jobs per target (see AGENTS.md). |
| Structure prediction (fallback) | **Chai-1** | 0.6.1+ (PyPI) | Apache 2.0, pip-installable. 77% PoseBusters RMSD <2 Å, comparable to AF3. Use when AF3 queue times are prohibitive. | Chai Discovery 2024, bioRxiv 2024.10.10.615955 | Requires GPU with bfloat16. Not as extensively validated as AF3. |
| Structure prediction (RNA-protein) | **Boltz-2** | 2.0+ (PyPI) | MIT license, pip-installable. Adds binding affinity prediction (r~0.6 vs FEP+). Specifically improved for RNA chains and DNA-protein complexes — use for RNase T2 + ApA substrate co-folding. ~1000× faster than FEP methods. | Passaro, Corso et al. 2025, bioRxiv PMC12262699 | Lags AF3 for antibody-antigen. RNA accuracy improvements over Boltz-1 not yet independently benchmarked on plant enzymes. |
...
| Stability prediction | FoldX | **4.1/5.1** (2025 revision) | Revised force field (Botte et al. 2025): Pearson R improved from 0.693 → 0.711, RMSD improved to 1.238 kcal/mol, AUC 0.78. **Now includes pH dependency for Asp, Glu, Lys, Arg, Tyr** — run PositionScan at pH 2.5, 3.5, and 5.0 to capture acid-enzyme effects. Training expanded to 2,484 high-quality mutations (FSD). | Botte et al. 2025, *Bioinformatics* 41(2):btaf064; Schymkowitz et al. 2005, *Nucleic Acids Res.* 33:W382 | **Destabilizing ~69%, stabilizing ~29%** (Buß et al. 2018 — not yet reassessed for revised version). Appropriate for classification, not quantitative engineering. Academic license required. Pass `--pH` flag for each pitcher pH value. |
